...
from BrainGNN_Pytorch.net.braingraphconv import MyNNConv
from GraphKAN.kan_layer import NaiveFourierKANLayer as KANLayer
...
class Network(torch.nn.Module):
    def __init__(self, indim, ratio, nclass, k=8, R=360):
        ...
        self.lin_in_1 = nn.Linear(self.dim3, self.dim4)
        self.kan_in_1 = KANLayer(self.dim4, self.dim4, 200)
        self.lin_in_2 = nn.Linear(self.dim4, self.dim5)
        self.kan_in_2 = KANLayer(self.dim5, self.dim6, 200)
        self.lin_out = nn.Linear(self.dim6, nclass)
    ...
    def forward(self, x, edge_index, batch, edge_attr, pos):
        ...
        return x,self.pool1.multiplier ,self.pool2.multiplier , torch.sigmoid(score1).view(x.size(0),-1), torch.sigmoid(score2).view(x.size(0),-1)
